            ...
            <el-row :gutter="40">
                ...
                <el-col :span="12">
                    ...
                    <div class="map-container">
                        <img
                            src="https://images.unsplash.com/photo-1572426841539-9b6a585a7f1d?q=80&w=2070&auto=format&fit=crop"
...
const onSubmit = async () => {
    ...
    // The entire function is a client-side event handler,
    // but to be absolutely safe from SSR issues, we ensure window is available.
    if (typeof window === 'undefined') return;

    try {
        await formRef.value.validate();

        const recipientEmail = 'joufson@joufson.com'; // Please replace with your actual email address
        const subject = `Website Inquiry from ${form.value.name}`;
        const body = `Name: ${form.value.name}
From Email: ${form.value.email}
...
${form.value.message}`;
        ...
        ElMessage({
            message: t('contact.redirecting_to_email'),
            type: 'success',
        });
        ...
        ElMessage({
            message: t('contact.validation_error'),
            type: 'error',
        });
